"""Phase 42 v35: L4 GPU validation (single GPU).

Tests checkpoint/restore on L4 (Ada Lovelace, sm_89, 24GB).
L4 is the most cost-effective inference GPU — validating here is critical.
Single GPU (TP=1) since L4 is typically used standalone.

Usage:
    modal run cuda_serializer/modal_multigpu_v35.py
"""

import modal, os
app = modal.App("vllm-ckpt-l4-v35")
...
try:
    ...
    print("=" * 60, flush=True)
    print("  PHASE 42 v35: L4 SINGLE GPU VALIDATION", flush=True)
    print("=" * 60, flush=True)
    for i, g in enumerate(gpu_info):
        print(f"  GPU {i}: {g['name']} ({g['memory_mib']}, sm_{g['compute_cap']}, driver {g['driver']})", flush=True)

    print(f"\n--- Starting vllm serve (Qwen2-7B, enforce-eager, TP=1) ---", flush=True)
    ...
    print(f"  TP: 1, PIDs: {len(pids)}", flush=True)
    ...
    result = {
        ...
        "model": MODEL, "cuda_graphs": False, "tp": 1,
...
def test_l4_v35():
    """Qwen2-7B + enforce-eager + TP=1 on L4."""
...
def main():
    print("Phase 42 v35: L4 single GPU validation")
    print("=" * 60)

    r = test_l4_v35.remote()
